...
# Configure GPU memory growth for LSTM (for Linux/WSL)
...
# # Set Thai font as default for Matplotlib
# font_path = Path.cwd() / "THSarabun.ttf"
# prop = fm.FontProperties(fname=font_path)
# plt.rcParams['font.family'] = prop.get_name()
# plt.rcParams['font.size'] = 12  # Set default font size if needed

# Define English mapping for file names
file_name_mapping = {
...
file_path = '/mnt/e/MayThesis2025/cleanned_datasets/(38t)โรงพยาบาลส่งเสริมสุขภาพตำลบ้านสบป้าด.csv'  # Example file path, can be changed dynamically
...
file_name = os.path.basename(file_path).replace(".csv", "")  # Remove .csv extension

english_file_name = file_name_mapping.get(file_name, file_name)  # Map to English name if available

# Check filename is in [ก-๙] Thai characters raise ValueError
if all([char >= 'ก' and char <= '๙' for char in file_name]):
    raise ValueError("Invalid filename: Must be in Thai characters [ก-๙]")


# Define output directory in the current working directory (for Jupyter Notebook)
output_dir = Path.cwd() / "outputs"
output_dir.mkdir(exist_ok=True)  # Create outputs folder if it doesn't exist
doc = Document()
...
add_table(doc, df_head_before, f"Sample of First 10 Rows (Before) - {file_name}", f"ตารางนี้แสดงตัวอย่าง 10 แถวแรกของข้อมูลดิบก่อนดำเนินการ ({file_name})")
...
add_figure(doc, output_dir / f"eda_heatmap_before_all_{english_file_name}.png", f"Missing Data Heatmap (Before Any Processing) - {file_name}", desc)
...
add_table(doc, correlation_df_before, f"Sorted Correlation Matrix with SO2 (Before) - {file_name}", f"ตารางนี้แสดงค่าสหสัมพันธ์กับ SO2 เรียงลำดับตามค่าสัมประสิทธิ์สัมพันธ์สัมบูรณ์ก่อนดำเนินการ ({file_name})")
...
add_figure(doc, output_dir / f"eda_corr_before_all_{english_file_name}.png", f"Correlation Matrix (Before Any Processing) - {file_name}", desc)
...
add_figure(doc, output_dir / f"eda_boxplot_before_all_{english_file_name}.png", f"Feature Outlier Distribution (Before Any Processing) - {file_name}", desc)
...
add_figure(doc, output_dir / f"eda_histogram_before_all_{english_file_name}.png", f"Feature Histograms (Before Any Processing) - {file_name}", desc)
...
for i, col in enumerate(df.columns):
    ...
    if not data.empty:
        ...
        plt.text(0.5, 0.5, 'No valid data (all NaN)', ha='center', va='center')
    # Add yearly labels (2010, 2011, 2012, ...)
...
add_figure(doc, output_dir / f"timeseries_plot_before_all_{english_file_name}.png", f"Timeseries Plot for All Features (Before Any Processing) - {file_name}", desc)
...
features_to_drop = [col for col in features_to_drop if col != 'SO2']  # Exclude SO2 from dropping
if features_to_drop:
...
# Update EDA after Removing Missing > 50% (Before Trim and Trim SO2)
doc.add_heading(f"Exploratory Data Analysis (After Removing Missing > 50%, Before Trim and Trim SO2) - {file_name}", 1)
...
# Missing Data Heatmap (After Removing Missing > 50%, Before Trim and Trim SO2)
plt.figure(figsize=(12, 6))
heatmap = sns.heatmap(df.isnull(), cbar=True, cmap='Blues', annot=False)
plt.title(f"Missing Data Heatmap (After Removing Missing > 50%, Excluding SO2, Before Trim and Trim SO2) - {english_file_name}")
plt.xlabel("Features")
plt.ylabel("Samples")
plt.savefig(output_dir / f"eda_heatmap_after_remove_{english_file_name}.png", dpi=300, bbox_inches='tight')
plt.close()
...
desc += f"วันที่เริ่มต้น: {df.index.min().strftime('%Y-%m-%d %H:%M')}, วันที่สิ้นสุด: {df.index.max().strftime('%Y-%m-%d %H:%M')}"
add_figure(doc, output_dir / f"eda_heatmap_after_remove_{english_file_name}.png", f"Missing Data Heatmap (After Removing Missing > 50%, Excluding SO2, Before Trim and Trim SO2) - {file_name}", desc)

# Correlation Matrix (After Removing Missing > 50%, Before Trim and Trim SO2)
...
add_table(doc, correlation_df_after_remove, f"Sorted Correlation Matrix with SO2 (After Removing Missing > 50%) - {file_name}", f"ตารางนี้แสดงค่าสหสัมพันธ์กับ SO2 เรียงลำดับตามค่าสัมประสิทธิ์สัมพันธ์สัมบูรณ์หลังลบฟีเจอร์ที่มี Missing > 50% (ไม่รวม SO2) ({file_name})")

plt.figure(figsize=(10, 8))
heatmap = sns.heatmap(corr_matrix_after_remove, annot=True, cmap='RdBu', center=0, vmin=-1, vmax=1, fmt='.2f')
plt.title(f"Correlation Matrix (After Removing Missing > 50%, Before Trim and Trim SO2) - {english_file_name}")
plt.xticks(rotation=45, ha='right')
plt.yticks(rotation=0)
plt.savefig(output_dir / f"eda_corr_after_remove_{english_file_name}.png", dpi=300, bbox_inches='tight')
plt.close()
...
if corr_max_after > 0.7:
    ...
    desc += "แสดงถึงความสัมพันธ์ที่ต่ำ "
desc += "การกระจายของค่าสหสัมพันธ์มีแนวโน้มสมมาตรตามแนวทแยง"
add_figure(doc, output_dir / f"eda_corr_after_remove_{english_file_name}.png", f"Correlation Matrix (After Removing Missing > 50%, Before Trim and Trim SO2) - {file_name}", desc)

# Feature Outlier Distribution (After Removing Missing > 50%, Before Trim and Trim SO2)
plt.figure(figsize=(15, 6))
boxplot = df.boxplot(figsize=(15, 6))
plt.xticks(rotation=45, ha='right')
plt.title(f"Feature Outlier Distribution (After Removing Missing > 50%, Before Trim and Trim SO2) - {english_file_name}")
plt.savefig(output_dir / f"eda_boxplot_after_remove_{english_file_name}.png", dpi=300, bbox_inches='tight')
plt.close()
...
add_figure(doc, output_dir / f"eda_boxplot_after_remove_{english_file_name}.png", f"Feature Outlier Distribution (After Removing Missing > 50%, Before Trim and Trim SO2) - {file_name}", desc)

# Feature Histograms (After Removing Missing > 50%, Before Trim and Trim SO2)
n_cols = 4
n_rows = (len(df.columns) + n_cols - 1) // n_cols
plt.figure(figsize=(15, n_rows * 3))
for i, col in enumerate(df.columns):
    ...
    plt.title(col)
plt.tight_layout()
plt.savefig(output_dir / f"eda_histogram_after_remove_{english_file_name}.png", dpi=300, bbox_inches='tight')
plt.close()
...
add_figure(doc, output_dir / f"eda_histogram_after_remove_{english_file_name}.png", f"Feature Histograms (After Removing Missing > 50%, Before Trim and Trim SO2) - {file_name}", desc)

# Trim Head/Tails Based on SO2
print("Step 6.5: Trimming Head/Tails Based on SO2...")
# Check if 'SO2' exists and is not all NaN
if 'SO2' in df.columns and df['SO2'].notna().any():
    # Find non-NaN SO2 values
    valid_so2 = df['SO2'].notna()
    if valid_so2.sum() > 0:  # Ensure there are valid SO2 values
        df = df[valid_so2]  # Trim to keep only rows with non-NaN SO2
...
# Update EDA after Trimming Based on SO2
...
# Missing Data Heatmap (After Trimming Based on SO2)
plt.figure(figsize=(12, 6))
heatmap = sns.heatmap(df.isnull(), cbar=True, cmap='Blues', annot=False)
plt.title(f"Missing Data Heatmap (After Trimming Based on SO2) - {english_file_name}")
plt.xlabel("Features")
plt.ylabel("Samples")
plt.savefig(output_dir / f"eda_heatmap_after_trim_so2_{english_file_name}.png", dpi=300, bbox_inches='tight')
plt.close()
...
desc += f"วันที่เริ่มต้น: {df.index.min().strftime('%Y-%m-%d %H:%M')}, วันที่สิ้นสุด: {df.index.max().strftime('%Y-%m-%d %H:%M')}"
add_figure(doc, output_dir / f"eda_heatmap_after_trim_so2_{english_file_name}.png", f"Missing Data Heatmap (After Trimming Based on SO2) - {file_name}", desc)

# Simple Data Sample (10 Rows) After Trimming/Remove Missing
print("Step 6.6: Showing Simple Data Sample (10 Rows) After Trimming/Remove Missing...")
df_head_after = df.head(10)
add_table(doc, df_head_after, f"Sample of First 10 Rows (After Trimming/Remove Missing) - {file_name}", f"ตารางนี้แสดงตัวอย่าง 10 แถวแรกของข้อมูลหลังลบฟีเจอร์ที่มี Missing > 50% และตัดทอนตาม SO2 ({file_name})")

# Data Information (After Trimming/Remove Missing)
...
# Statistical Summary Table (After Trimming/Remove Missing)
print("Step 6.8: Creating Statistical Summary Table (After Trimming/Remove Missing)...")
doc.add_heading(f"Statistical Summary Table (After Trimming/Remove Missing) - {file_name}", 1)
...
for col in df.columns:
    if col != 'Datetime':  # Skip Datetime column if it's not numerical
...
# Initialize features and scalers with default values
features = None
scaler_features = None
scaler_target = None

# Process data and set features/scalers
if len(df) > 0:  # Use df after removing missing > 50% (excluding SO2) and trimming based on SO2
    ...
    if len(df_trimmed) > 0:
        # Check if 'SO2' exists in df_trimmed
        if 'SO2' in df_trimmed.columns:
            features = [col for col in df_trimmed.columns if col != 'SO2']  # Exclude SO2 from features
            ...
            doc.add_paragraph(desc)
            features = [col for col in df.columns if col != 'SO2']  # Use df after removing missing > 50%, exclude SO2
            scaler_features = MinMaxScaler()
            scaler_target = MinMaxScaler()
            # Check and handle NaN before normalization
            if df[features].isnull().any().any() or df['SO2'].isnull().any():
                df[features] = df[features].fillna(0)  # Replace NaN with 0 or use another strategy
...
# Ensure features is defined before calling prepare_sequences
seq_length = 24
pred_length = 24
if features is not None:  # Check if features is defined
    ...
    doc.save(output_dir / f"report_{file_name}_error.docx")
    exit()

# Prepare flat data for XGBoost SHAP
if features is not None:
    # Check for NaN in flat data
    if df[features].isnull().any().any() or df['SO2'].isnull().any():
        df[features] = df[features].fillna(0)  # Replace NaN with 0 or use another strategy
        df['SO2'] = df['SO2'].fillna(0)
...
for fold, (train_idx, test_idx) in enumerate(tscv.split(X_full)):
    ...
    # Check for NaN in y_test and y_pred before inverse transform
    if np.any(np.isnan(y_test)) or np.any(np.isnan(y_pred)):
        ...
        # Optionally, fill NaN with a default value (e.g., 0 or mean)
    ...
    add_figure(doc, output_dir / f"fold_{fold+1}_loss_{english_file_name}.png", f"Fold {fold+1} Loss - {file_name}", desc)
    ...
    add_figure(doc, output_dir / f"fold_{fold+1}_pred_{english_file_name}.png", f"Fold {fold+1} Predictions - {file_name}", desc)
...
if features is not None:
    ...
    # Check for NaN in flat data before training XGBoost
    ...
    shap_explanation = shap.Explanation(
        ...
        base_values=explainer.expected_value,  # Base value (expected value from the model)
    ...
    # Ensure the file paths are correct and the files exist before adding to the document
    ...
    add_figure(doc, beeswarm_plot_path, f"SHAP Tree Summary Plot (Beeswarm) - {file_name}", desc)
    ...
    add_table(doc, feature_importance, f"Feature Importance Ranking (SHAP Tree via XGBoost) - {file_name}", desc)
else:
...
if features is not None and len(X_full) > 0:
    last_data = X_full[-1:]
    # Check for NaN in last_data before prediction
    ...
    add_figure(doc, output_dir / f"final_pred_{english_file_name}.png", f"Next 24-Hour Prediction (LSTM) - {file_name}", desc)
else:

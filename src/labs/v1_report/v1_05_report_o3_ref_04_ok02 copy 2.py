...
class DynamicInputDataset:
    ...
    def clean_data(self, df):
        """
        Clean and validate environmental data based on physically realistic ranges.
        Values outside these ranges are set to NaN.
        """
        # Define valid ranges for each parameter
        valid_ranges = DynamicInputDataset.valid_ranges

        # Create a copy of the dataframe to avoid modifying the original
        df_cleaned = df.copy()

        # Apply cleaning for each column that exists in the dataframe
        ...
        for col in valid_ranges.keys():
            if col in df_cleaned.columns:
                invalid_count = df_cleaned[col].isna().sum() - df[col].isna().sum()
                cleaning_stats[col] = invalid_count
    ...
    def process_file(self, file_path):
        ...
        df_cleaned, cleaning_stats = self.clean_data(df)

        # Document cleaning results
        doc.add_heading(f"Data Cleaning Results - {file_name}", 1)
        doc.add_paragraph("Applied cleaning rules:")
        valid_range = DynamicInputDataset.valid_ranges
        for param, (min_val, max_val) in valid_range.items():
            doc.add_paragraph(f"- {param}: {min_val} to {max_val}")

        cleaning_table = pd.DataFrame(
            [(col, count) for col, count in cleaning_stats.items()],
            columns=["Parameter", "Invalid Values Replaced"],
        )
        add_table(
            doc,
            cleaning_table,
            f"Cleaning Statistics - {file_name}",
            f"Number of values replaced with NaN due to being outside valid ranges ({file_name})",
        )
        ...
        if len(df) > 0:
            ...
            if len(df_trimmed) > 0:
                if "SO2" in df_trimmed.columns:
                    ...
                    # Hypothesis: SO2 is the target, other columns are features
        ...
        for fold, (train_idx, test_idx) in enumerate(tscv.split(X_full)):
            ...
            if fold < self.CONST.N_SPLITS - 1:
                continue
            doc.add_heading(f"Fold {fold + 1} - {file_name}", 2)
        ...
        # Around line 1034, after all the fold-specific visualizations

        print("Step 8.99: Creating Full Dataset Prediction Graph...")
        doc.add_heading(f"Full Dataset Prediction Visualization - {file_name}", 1)
        ...
        # Create a DataFrame to align predictions with actual data
        prediction_dates = df.index[seq_length : seq_length + len(all_predictions)]
        ...
        plt.title(f"Full Dataset: Actual vs Predicted SO2 - {english_file_name}")
        plt.legend()
        plt.xlabel("Date")
        plt.ylabel("SO2")
        ...
        # Save the figure
        plt.savefig(
            self.output_dir / f"full_dataset_prediction_{english_file_name}.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        if n_years > 1:
            ...
            for i, year in enumerate(years[:min_year]):  # Limit to first 8 years if more
                ...
                plt.title(f"Year {year}: Actual vs Predicted SO2 - {english_file_name}")
                plt.legend()
                plt.ylabel("SO2")
                plt.grid(True, alpha=0.3)
                ...
                if not year_data["Actual"].dropna().empty and not year_data["Predicted"].dropna().empty:
                    ...
                    plt.text(
                        0.02,
                        0.85,
                        f"MSE: {mse_year:.4f}, MAE: {mae_year:.4f}, R²: {r2_year:.4f}",
            ...
            plt.tight_layout()
            plt.savefig(
                self.output_dir / f"yearly_predictions_{english_file_name}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()

        # Add metrics table to the report
        full_metrics = pd.DataFrame(
            [
                {"Metric": "MSE", "Value": mse_full},
                {"Metric": "MAE", "Value": mae_full},
                {"Metric": "R²", "Value": r2_full},
            ]
        )

        # Add the visualization to the document
        desc_full = f"กราฟนี้แสดงการเปรียบเทียบระหว่างค่า SO2 จริงและค่าพยากรณ์สำหรับชุดข้อมูลทั้งหมด ({file_name}) "
        desc_full += f"MSE: {mse_full:.4f}, MAE: {mae_full:.4f}, R²: {r2_full:.4f} "

        if r2_full > 0.8:
            desc_full += "ค่า R² สูงแสดงถึงความแม่นยำที่ดีของโมเดลในการพยากรณ์แนวโน้มทั่วไป "
        elif r2_full < 0.5:
            desc_full += "ค่า R² ต่ำกว่า 0.5 แสดงถึงความแม่นยำที่ยังไม่เพียงพอสำหรับชุดข้อมูลทั้งหมด "
        else:
            desc_full += "ค่า R² อยู่ในระดับปานกลาง "

        desc_full += f"ชุดข้อมูลครอบคลุมตั้งแต่ {prediction_df.index.min().strftime('%Y-%m-%d')} ถึง {prediction_df.index.max().strftime('%Y-%m-%d')} "
        desc_full += f"รวมจำนวนทั้งสิ้น {len(prediction_df)} จุดข้อมูล"

        add_figure(
            doc,
            self.output_dir / f"full_dataset_prediction_{english_file_name}.png",
            f"Full Dataset Prediction - {file_name}",
            desc_full,
        )
        ...
        if n_years > 1:
            ...
            add_figure(
                doc,
                self.output_dir / f"yearly_predictions_{english_file_name}.png",
                f"Yearly Predictions - {file_name}",
                desc_yearly,
            )
        ...
        add_table(
            doc,
            full_metrics,
            f"Full Dataset Prediction Metrics - {file_name}",
            f"ตารางนี้แสดงค่าเมตริกสำหรับการพยากรณ์ชุดข้อมูลทั้งหมด MSE: {mse_full:.4f}, MAE: {mae_full:.4f}, R²: {r2_full:.4f} ({file_name})",
        )

        # Free up memory
        del final_model, prediction_df, full_dataset
        gc.collect()
        # endregion

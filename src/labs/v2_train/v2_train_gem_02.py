...
def load_and_preprocess(file_path):
    ...
    if df.isnull().any().any():
        ...
        if df.isnull().any().any():
            raise ValueError(
                "NaNs persist after ffill and bfill.  Review data cleaning."
            )
...
def create_sequences(data, features, target, sequence_length, forecast_horizon):
    ...
    for i in range(len(data) - sequence_length - forecast_horizon + 1):
        ...
        y.append(
            data[target]
            .iloc[i + sequence_length : i + sequence_length + forecast_horizon]
            .values
        )
...
def build_lstm_model(
    sequence_length, num_features, forecast_horizon, dropout_rate=0.2, lstm_units=64
):
...
def train_evaluate_model(
    model, X_train, y_train, X_test, y_test, batch_size, epochs, scaler_target
):
...
if __name__ == "__main__":
    ...
    df_scaled, features, target, scaler_features, scaler_target = (
        feature_selection_and_scaling(df)
    )
    X, y = create_sequences(
        df_scaled, features, target, sequence_length, forecast_horizon
    )
    print(f"Shape of X: {X.shape}")
    print(f"Shape of y: {y.shape}")
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=test_size_proportion, shuffle=False
    )
    model = build_lstm_model(sequence_length, len(features), forecast_horizon)
    print(model.summary())
    history, y_pred_inv, y_test_inv, best_model = train_evaluate_model(
        model, X_train, y_train, X_test, y_test, batch_size, epochs, scaler_target
    )
